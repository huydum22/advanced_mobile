export const background1 =
  'https://itedu.me/static/media/bg-sliders-2.584f4b39.jpg';
export const background2 =
  'https://itedu.me/static/media/bg-sliders-1.6bc249ae.jpg';
export const background3 =
  'https://itedu.me/static/media/bg-sliders-3.ad2511e4.jpg';

export const categoryImage = [
  'https://pluralsight.imgix.net/course-images/django-user-authentication-v1.png',
  'https://pluralsight.imgix.net/course-images/collection-powerupsql-v1.png',
  'https://pluralsight.imgix.net/course-images/getting-started-machine-learning-matlab-v1.png',
  'https://pluralsight.imgix.net/course-images/microsoft-azure-data-securing-v1.png',
  'https://pluralsight.imgix.net/course-images/exploring-partnering-tech-leads-v1.png',
  'https://pluralsight.imgix.net/course-images/visualizing-network-traffic-wireshark-v1.jpg',
  'https://pluralsight.imgix.net/course-images/audience/software-development.jpg',
  'https://pluralsight.imgix.net/course-images/android-application-basics-understanding-v1.jpg',
];

export const urlAvatarDefault =
  'https://c7.uihere.com/files/592/884/975/programmer-computer-programming-computer-software-computer-icons-programming-language-avatar.jpg';
